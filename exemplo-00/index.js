...
async function trainModel(inputXs, outputYs) {
    ...
    await model.fit(
        ...
        outputYs,
        {
            ...
            epochs: 100, // quantidade de veses que vai rodar no dataset
...
async function predict(model, pessoa) {
    ...
    //fazendo a previsão (output será vetor de 3 probabilidades)
...
// Normalizando idede e usando mesmo padrão de treino
// Exemplo: idade_min = 25, idade_max = 40, então (35 - 25) / (40 - 25 ) = 0.66

const pessoaTensorNormalizado = [
    [
        0.66, // idade normalizada
        1,    // cor azul
        0,    // cor vermelho
        0,    // cor verde
...
const precictions = await predict(model, pessoaTensorNormalizado)
const results = precictions.
sort((a, b) => b.prob - a.prob)
.map(p => `categoria: ${labelsNomes[p.index]} - probabilidade: ${(p.prob * 100).toFixed(2)}%`)
.join('\n')
console.log(results)